defmodule MiataBot.CopyPastaWorker do
  ...
  def handle_cast({:activity, message}, @max) do
    ...
    case Enum.random([:pastebin, :messages]) do
      :pastebin ->
        do_pastebin(message)

      :messages ->
        Logger.info("using local cache of copypasta")
        do_messages(message)
    end
  ...
  defp do_pastebin(message, attempts \\ 0)

  defp do_pastebin(message, 10) do
    do_messages(message)
  end

  defp do_pastebin(message, attempts) do
    paste = PastebinRandomizer.good_paste()
    Logger.info("getting copypasta from pastbin #{paste}")
    copypasta = PastebinRandomizer.get(paste)

    case Nostrum.Api.create_message(message.channel_id, copypasta) do
      {:error, reason} ->
        Logger.error("copypasta randomizer error: #{inspect(reason)}")
        do_pastebin(message, attempts + 1)
